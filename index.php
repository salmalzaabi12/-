...
global $IGMail;
        $mail = strtolower($mail);
            ...
            } elseif(strpos($mail, 'yahoo.com') and $e == "yahoo.com"){
...
function checkRU($mail){
  ...
  $html = file_get_contents("https://auth.mail.ru/api/v1/pushauth/info?login=$mail");
...
function checkYahoo($user){
    ...
    $b = curl_exec($c); 
    if(strstr($b,'messages.ERROR_INVALID_USERNAME') or strstr($b,'messages.INVALID_USERNAME') or strstr($b,'messages.ERROR_NOTFOUND') or strstr($b,'INVALID_USERNAME')){
      return true;
    }else{ 
      return false; 
    } 
